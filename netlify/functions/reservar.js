...
exports.handler = async (event) => {
    ...
    try {
        ...
        // Validación de datos básicos (como tu código original)
        if (!email || !consultorio || !fecha || (hora !== 0 && !hora)) {
            return { statusCode: 400, body: JSON.stringify({ error: 'Faltan datos' }) };
        }

        // --- VALIDACIÓN DE SÁBADOS Y DOMINGOS ---
        const fechaObj = new Date(fecha + 'T00:00:00');
        const dia = fechaObj.getDay();
        if (dia === 0) return { statusCode: 400, body: JSON.stringify({ error: 'Domingos cerrado' }) };
        if (dia === 6 && hora >= 15) return { statusCode: 400, body: JSON.stringify({ error: 'Sábados solo hasta las 15hs' }) };
