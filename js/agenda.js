...
async function cargarBotonesConsultorios(container) {
    ...
    disponibles.forEach(num => {
        ...
        btn.style = "padding: 25px; font-size: 1.3rem; font-weight: bold; border-radius:12px;";
...
function elegirFecha(num, container) {
    ...
    container.innerHTML = `
        <div style="background:#f0f7ff; padding:20px; border-radius:10px;">
            ...
            <input type="date" id="input-fecha-agenda" class="search-input" style="padding:15px; font-size:1.2rem; margin: 1em 0; display:block; width:100%; max-width:400px;">
            <button id="btn-volver-cons" class="btn btn-secondary">← Volver</button>
        </div>
    ...
    input.onchange = (e) => {
        ...
        if (fechaSel.getDay() === 0) {
            alert("Los domingos el centro está cerrado.");
            e.target.value = "";
            return;
        }
...
async function cargarHorarios(targetContainer) {
    targetContainer.innerHTML = '<p>⌛ Buscando disponibilidad...</p>';
    try {
